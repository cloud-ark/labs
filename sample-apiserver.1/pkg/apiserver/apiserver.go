	...
	// Read file location from environment variable
     ...
     for _, crd := range lines {
     	if crd != "" {
     		fmt.Printf("CRD:%v\n", crd)
			fmt.Printf("========== 8 ===============\n")
		    path := "/apis/wardle.k8s.io/v1alpha1/namespaces/default/" + crd

			ws := installProvenanceWebService()
			ws.Path(path).
				Consumes(restful.MIME_JSON, restful.MIME_XML).
				Produces(restful.MIME_JSON, restful.MIME_XML)
			ws.Route(ws.GET("/{crd-id}/compositions").To(getCompositions))
			s.GenericAPIServer.Handler.GoRestfulContainer.Add(ws)
		}
	}
...
func installProvenanceWebService() *restful.WebService {
	...
	ws.ApiVersion("wardle.k8s.io/v1alpha1")
	return ws
}

func addCompositionRoute(ws *restful.WebService) {
}

func getCompositions(request *restful.Request, response *restful.Response) {
	fmt.Printf("========== AAAAA ===============\n")
	id := request.PathParameter("crd-id")
	stringToReturn := "Hello there - THis is compositon of CRD ID:%s" + id + " -- Pod1 Pod2 Service2\n"
